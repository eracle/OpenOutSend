"""What a first run collects: the campaign's fields, the model, the operator, a mailbox.
...
def unconfigured_environment(monkeypatch):
    ...
    for variable in [*CONFIG_ENV.values(), *LLM_ENV.values(), *OPERATOR_ENV.values(),
...
def test_the_environment_is_enough_to_be_ready(headless, smtp, monkeypatch):
    ...
    assert SiteConfig.load().llm_api_key == "sk-ada"
...
def test_the_campaign_fields_are_asked_for_on_a_terminal(terminal, smtp, stored_llm, monkeypatch):
    monkeypatch.setenv(MAILBOX_ENV["address"], "ada@corp.com")
    monkeypatch.setenv(MAILBOX_ENV["password"], "app-pw")
    monkeypatch.setenv(SIGNATURE_ENV, "")
    UserFactory()
