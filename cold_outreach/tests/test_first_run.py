"""What a first run collects: the campaign's fields, the operator, and a mailbox.
...
_EVERYTHING = {
    CONFIG_ENV["product_docs"]: "A self-hosted lead finder.",
    CONFIG_ENV["campaign_target"]: "Founders at small B2B software companies.",
...
def unconfigured_environment(monkeypatch):
    ...
    for variable in [*CONFIG_ENV.values(), *OPERATOR_ENV.values(), *MAILBOX_ENV.values(),
                     *TRANSPORT_ENV.values(), SIGNATURE_ENV]:
...
def _bare_campaign():
    return CampaignFactory(product_docs="", campaign_target="")
...
def test_a_headless_run_names_everything_it_is_missing_at_once(headless):
    ...
    with pytest.raises(OutsendError) as stopped:
        ensure_ready(_bare_campaign())

    message = str(stopped.value)
    for variable in [CONFIG_ENV["product_docs"], CONFIG_ENV["campaign_target"],
...
def test_the_environment_is_enough_to_be_ready(headless, smtp, monkeypatch):
    ...
    assert campaign.product_docs and campaign.campaign_target
...
def test_nothing_is_asked_of_a_timer_that_already_has_it_all(headless, smtp):
    """An install that is set up costs a send pass nothing — not even an SMTP round trip."""
    UserFactory()
    maillog.mailbox()

    ensure_ready(CampaignFactory())
...
def test_the_operator_is_asked_once_and_never_again(terminal, smtp, monkeypatch):
...
def test_an_operator_who_declines_a_bcc_address_is_still_complete(terminal, smtp, monkeypatch):
    monkeypatch.setenv(MAILBOX_ENV["address"], "ada@corp.com")
    monkeypatch.setenv(MAILBOX_ENV["password"], "app-pw")
    monkeypatch.setenv(SIGNATURE_ENV, "")
...
def test_the_password_is_asked_for_without_echoing_it(terminal, smtp, monkeypatch):
    monkeypatch.setenv(SIGNATURE_ENV, "")
    UserFactory()
...
def test_a_declined_sign_off_sticks_and_is_not_asked_again(terminal, smtp):
...
def test_the_campaign_fields_are_asked_for_on_a_terminal(terminal, smtp, monkeypatch):
    monkeypatch.setenv(MAILBOX_ENV["address"], "ada@corp.com")
    monkeypatch.setenv(MAILBOX_ENV["password"], "app-pw")
    monkeypatch.setenv(SIGNATURE_ENV, "")
    UserFactory()
    ...
    campaign.refresh_from_db()
    assert (campaign.product_docs, campaign.campaign_target, campaign.booking_link) == (
        "A lead finder", "Founders", "cal.com/ada")


def test_nothing_it_asks_reaches_stdout(terminal, smtp, capsys, monkeypatch):
