...
@pytest.fixture
def waiting(campaign):
    """Three leads waiting for a first email — a pool the goal cannot drain by accident."""
    for i in range(3):
        DealFactory(campaign=campaign, lead=LeadFactory(email=f"lead{i}@acme.com"))
    return campaign
...
def _run(campaign, goal, passes, slept, opening_in=timedelta(minutes=4)):
    ...
    with patch("cold_outreach.send_job.run_send_pass", side_effect=passes) as pass_, \
        ...
        result = run_send_job(campaign, goal, sleep=slept.append)
    return result, pass_
...
def test_it_keeps_passing_until_the_goal_is_open(waiting, slept):
    ...
    result, pass_ = _run(waiting, 3, [PassResult(opened=1)] * 3, slept)
...
def test_only_openers_count_toward_the_goal(waiting, slept):
    ...
    """
    result, _ = _run(waiting, 2, [PassResult(answered=4, followed_up=3),
                                  PassResult(opened=1),
...
def test_the_run_stops_the_moment_the_goal_is_met(waiting, slept):
    ...
    result, pass_ = _run(waiting, 3, [PassResult(opened=1), PassResult(opened=2),
...
def test_it_waits_out_the_spacing_clock_after_a_send(waiting, slept):
    ...
    result, _ = _run(waiting, 2, [PassResult(opened=1), PassResult(opened=1)], slept,
...
def test_a_long_wait_is_slept_in_slices_so_the_mail_is_still_read(waiting, slept):
    ...
    result, pass_ = _run(waiting, 2, passes, slept, opening_in=timedelta(hours=12))
...
def test_an_idle_pass_never_spins_even_when_a_box_is_free(waiting, slept):
    ...
    _run(waiting, 2, [PassResult(), PassResult(opened=2)], slept,
...
def test_a_drained_pool_ends_the_run_instead_of_waiting_for_leads(campaign, slept):
    """The one gate no clock resolves: leads arrive by a separate invocation."""
    result, _ = _run(campaign, 5, [PassResult(opened=1)], slept)
...
def test_no_mailbox_ends_the_run(waiting, slept):
    result, _ = _run(waiting, 5, [PassResult()], slept, opening_in=None)
...
def test_two_refused_passes_end_the_run(waiting, slept):
    ...
    result, pass_ = _run(waiting, 5, [PassResult(failed=1), PassResult(failed=1),
...
def test_one_failure_is_a_blip_and_the_next_pass_is_the_retry(waiting, slept):
    result, _ = _run(waiting, 2, [PassResult(failed=1), PassResult(opened=2)], slept)
...
def test_a_failure_alongside_an_opener_does_not_count_as_a_refusal(waiting, slept):
    ...
    result, _ = _run(waiting, 3, [PassResult(opened=1, failed=1),
...
def test_an_interrupt_hands_back_what_was_already_sent(waiting, slept):
    ...
    with patch("cold_outreach.send_job.run_send_pass",
        ...
        result = run_send_job(waiting, 9, sleep=slept.append)
...
def test_the_same_answer_is_not_reprinted_every_slice(waiting, slept, caplog):
    ...
    with caplog.at_level("INFO"):
        _run(waiting, 1, passes, slept, opening_in=timedelta(hours=12))
